...
export default function GitLabProjects() {
  const DEFAULT_PAGINATION: { page: number; size: number } = {
    page: 1,
    size: 10,
  };

  const [projects, setProjects] = useState<Record<string, any> | null>(null);
  ...
  const lastFetchRequest = useRef<Promise<any> | null>(null);
  ...
  const onToggleProject = (
    _: any,
    { value: projectId, checked }: { value: string; checked?: boolean },
  ) => {
    setToggling(projectId, true);

    let data: { project_id: string; hook_id?: string | number } = {
  ...
  if (!projects) {
    ...
    return (
      <>
        <Search onChange={onSearchFilterChange} loading={fetchingProjects} />
        {!isEmpty(projects) ? (
          <>
            <List>
            ...
            <div className={styles.pagination}>
              <Pagination
                ...
                onPageChange={(_, { activePage }) => {
                  setPagination({ ...pagination, page: Number(activePage) });
                }}

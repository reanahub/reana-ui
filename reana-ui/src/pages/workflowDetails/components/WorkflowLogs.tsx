...
function EngineLogs({ logs, workflowStatus }: EngineLogsProps) {
  const isExecuting = NON_FINISHED_STATUSES.includes(workflowStatus as any);
  return logs ? (
...
function ServiceLogs({ isDask, components, workflowStatus }: ServiceLogsProps) {
  const isExecuting = NON_FINISHED_STATUSES.includes(workflowStatus as any);
...
function JobLogs({ logs }: JobLogsProps) {
      ...
      <section className={styles["step-info"]}>
        ...
        {log && (
          <div className={styles["step-tags"]}>
            <Label basic color={statusMapping[log.status].color as any}>
              {log.status}
              {log.duration && (
...
export default function WorkflowLogs({
  ...
  return loading ? (
    ...
    <ServiceLogs
      isDask={(workflow.services as any[])?.length > 0}
      workflowStatus={workflow.status}
      components={
        Object.keys(serviceLogs as any).length === 0
          ? []
          : (Object.values(serviceLogs as any)[0] as ServiceComponent[])
      }
    />
  ) : (

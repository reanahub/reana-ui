...
interface ValidationErrors {
  ...
  parameters: Array<{ key: string | null; value: string | null }>;
}
...
const LauncherBadgeCreator = () => {
  ...
  const [launcherURL, setLauncherURL] = useState<string | null | undefined>();
  ...
  const handleDeleteParam = (index: number, paramKey: string) => {
    // Clear the validation error relative to the parameter that has been deleted
    setValidationErrors((prevErrors) => ({
      ...prevErrors,
      parameters: (prevErrors.parameters as any).toSpliced(index, 1),
    }));
    setLauncherData((prevData) => {
